...
# Add missing dataloader assignment
dataloader = train_loader

# Evaluate wrong predictions and add them to TensorBoard
with torch.no_grad():
    for data in dataloader:  # Veri kümesini döngüsü
        inputs, labels = data
        ...
        for i in range(len(wrong_mask)):
            if wrong_mask[i]:
                ...
                writer.add_text(f'True_Label_{i}', str(labels[i].item()), global_step=i)
                writer.add_text(f'Predicted_Label_{i}', str(predicted[i].item()), global_step=i)
